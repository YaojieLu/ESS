...
dvsf <- function(ca, k, MAP){
  ...
  res <- c(integralfnoc$abs.error/integralfnoc$value*100, averwp1$abs.error/averwp1$value*100, averE$abs.error/averE$value*100, averA$abs.error/averA$value*100, averm$abs.error/averm$value*100, avercica$abs.error/avercica$value*100, fL, averw, averE$value, averA$value, averm$value, avercica$value)
  return(res)
}
...
# Initialize
dvs <- matrix(nrow=nrow(env), ncol=12)
...
# Collect results
res <- cbind(env, dvs)
colnames(res) <- c("ca", "k", "MAP", "integralfnoc rel error (%)", "averwp1 rel error (%)", "averE rel error (%)", "averA rel error (%)", "averm rel error (%)", "avercica rel error (%)", "fL", "averw", "averE", "averA", "averm", "avercica") 
